  ...
  return (
    <a href={`/product/${slug}`} className="relative mt-2 border border-white p-1">
      <div className="absolute top-0 left-0 z-10 flex flex-col items-start">
        {name && <h3 className="border border-gray-200 bg-white py-1 px-2 text-xs font-medium text-black md:py-2 md:px-4 md:text-xl">{name}</h3>}
        {prices && (
      ...
      {images && <img className="h-full bg-white object-contain" loading="lazy" width={1200} height={1200} src={relativizeURL(images[0].url)} />}
    </a>
  )
}
